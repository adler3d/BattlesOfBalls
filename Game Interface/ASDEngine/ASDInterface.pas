...
type
  ...
  TASDObject = class(TInterfacedObject, IASDObject)
  private
    FASDObjectID: Integer;
  public
  ...
  IMouse = interface(IASDObject)
    ...
    property OnMouseMove: TMouseMoveEvent read GetOnMouseMove write
      SetOnMouseMove;
  end;
  ...
  {IStringStream = interface(IStream)
...
var
  FASDNextID: Integer;
  FASDCountOBJ: Integer;

function ASDNextID: Integer;
begin
  Result := FASDNextID;
  Inc(FASDNextID);
  Inc(FASDCountOBJ);
...
procedure ASDFreeID(ID: Integer);
begin
  Dec(FASDCountOBJ);
end;

function TASDObject.ASDAllOBJ: Integer;
begin
  Result := FASDNextID;
end;

function TASDObject.ASDCountOBJ: Integer;
begin
  Result := FASDCountOBJ;
end;

function TASDObject.ASDObjectID: Integer;
begin
  Result := FASDObjectID;
end;

function TASDObject.ASDName: string;
begin
  Result := ClassName;
end;

constructor TASDObject.CreateEx;
begin
  inherited Create;
  FASDObjectID := ASDNextID;
  FRefCount := MAXWORD;
  {if Log <> nil then
    Log.Print(PChar(ASDName + '.Create ID ' + IntToStr(FASDObjectID)));}
end;

destructor TASDObject.Destroy;
begin
  {if Log <> nil then
    Log.Print(PChar(ASDName + '.Destroy ID ' + IntToStr(FASDObjectID)));}
  inherited;
end;

procedure TASDObject.UnLoad;
begin
  FRefCount := 0;
  ASDFreeID(FASDObjectID);
  //Destroy;
end;
